...
FileLogger::FileLogger(const std::string& file, FileLoggerSpiltUnit unit,
                       uint16_t max_store_days, uint16_t time_offset)
    : _file_name(file), _time_offset(time_offset), _spilt_unit(unit) {
  if (unit == FLSU_HOUR) {
