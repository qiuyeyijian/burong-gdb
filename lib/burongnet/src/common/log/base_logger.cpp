...
static uint32_t FormatLog(const char* file, uint32_t line, const char* level,
...
static uint32_t FormatLog(const char* file, uint32_t line, const char* level,
                          const char* content, va_list list, char* buf,
                          uint32_t len) {
  uint32_t curlen = 0;
...
BaseLogger::BaseLogger(uint16_t cache_size, uint16_t block_size)
    : _level(LL_INFO), _cache_size(cache_size), _block_size(block_size) {
  _allocter = MakeNormalAlloterPtr();
}

BaseLogger::~BaseLogger() { SetLevel(LL_NULL); }
...
void BaseLogger::Debug(const char* file, uint32_t line, const char* content,
                       va_list list) {
  if (!(_level & LLM_DEBUG)) {
    return;
  }
...
void BaseLogger::Info(const char* file, uint32_t line, const char* content,
                      va_list list) {
  if (!(_level & LLM_INFO)) {
    return;
  }
...
void BaseLogger::Warn(const char* file, uint32_t line, const char* content,
                      va_list list) {
  if (!(_level & LLM_WARN)) {
    return;
  }
...
void BaseLogger::Error(const char* file, uint32_t line, const char* content,
                       va_list list) {
  if (!(_level & LLM_ERROR)) {
    return;
  }
...
void BaseLogger::Fatal(const char* file, uint32_t line, const char* content,
                       va_list list) {
  if (!(_level & LLM_FATAL)) {
    return;
  }
...
LogStreamParam BaseLogger::GetStreamParam(LogLevel level, const char* file,
                                          uint32_t line) {
