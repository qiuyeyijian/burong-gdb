...
LogStream::LogStream(const LogStreamParam& param)
    : _log(param.first), _call_back(param.second) {}
...
LogStream& LogStream::operator<<(bool v) {
  ...
  char c = v ? '1' : '0';
  _log->_len +=
      snprintf(_log->_log + _log->_len, __log_block_size - _log->_len, "%c", c);
  return *this;
}
...
LogStream& LogStream::operator<<(int8_t v) {
  ...
  _log->_len +=
      snprintf(_log->_log + _log->_len, __log_block_size - _log->_len, "%d", v);
  return *this;
}
...
LogStream& LogStream::operator<<(uint8_t v) {
  ...
  _log->_len +=
      snprintf(_log->_log + _log->_len, __log_block_size - _log->_len, "%u", v);
  return *this;
}
...
LogStream& LogStream::operator<<(int64_t v) {
  CHECK_CONTINUE()
#ifdef __win__
  _log->_len += snprintf(_log->_log + _log->_len, __log_block_size - _log->_len,
                         "%I64d", v);
#else
  _log->_len += snprintf(_log->_log + _log->_len, __log_block_size - _log->_len,
                         "%ld", v);
...
LogStream& LogStream::operator<<(const char* v) {
  ...
  _log->_len +=
      snprintf(_log->_log + _log->_len, __log_block_size - _log->_len, "%s", v);
  return *this;
}
...
LogStream& LogStream::operator<<(char v) {
  ...
  _log->_len +=
      snprintf(_log->_log + _log->_len, __log_block_size - _log->_len, "%c", v);
  return *this;
}
