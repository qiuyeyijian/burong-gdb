...
Address::Address() : Address(AT_IPV6) {}

Address::Address(AddressType at) : _address_type(at), _ip(""), _port(0) {}

Address::Address(AddressType at, const std::string& ip, uint16_t port)
    : _address_type(at), _ip(ip), _port(port) {}

Address::Address(const Address& addr)
    : _address_type(addr._address_type), _ip(addr._ip), _port(addr._port) {}
...
bool operator==(const Address& addr1, const Address& addr2) {
  return addr1._ip == addr2._ip && addr1._port == addr2._port &&
         addr1._port != 0;
}
