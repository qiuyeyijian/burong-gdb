...
SysCallInt32Result OsHandle::Write(int64_t sockfd, const char *data,
                                   uint32_t len) {
...
SysCallInt32Result OsHandle::Writev(int64_t sockfd, Iovec *vec,
                                    uint32_t vec_len) {
  DWORD flags = 0;
  DWORD send_bytes = 0;
  int32_t ret = WSASend((SOCKET)sockfd, (LPWSABUF)vec, (DWORD)vec_len,
                        &send_bytes, flags, nullptr, nullptr);
...
SysCallInt32Result OsHandle::Recv(int64_t sockfd, char *data, uint32_t len,
                                  uint16_t flag) {
...
SysCallInt32Result OsHandle::Readv(int64_t sockfd, Iovec *vec,
                                   uint32_t vec_len) {
  DWORD flags = 0;
  DWORD send_bytes = 0;
  int32_t ret = WSARecv((SOCKET)sockfd, (LPWSABUF)vec, (DWORD)vec_len,
                        &send_bytes, &flags, nullptr, nullptr);
  if (ret == SOCKET_ERROR) {
    return {(int32_t)send_bytes, WSAGetLastError()};
  }
