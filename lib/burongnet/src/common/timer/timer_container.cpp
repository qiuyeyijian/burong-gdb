...
bool TimerContainer::AddTimer(std::weak_ptr<TimerSlot> t, uint32_t time,
                              bool always) {
  if (time >= _timer_max) {
    return false;
  }
...
bool TimerContainer::RmTimer(std::weak_ptr<TimerSlot> t) {
  ...
  if (type == TimeUnit2TimeType(_time_unit)) {
    ...
    for (auto timer = timer_list->second.begin();
         timer != timer_list->second.end(); ++timer) {
...
bool TimerContainer::Empty() { return _bitmap.Empty(); }
...
bool TimerContainer::InnerAddTimer(std::shared_ptr<TimerSlot> ptr,
                                   uint32_t time) {
  if (time > _timer_max) {
    return false;
  }
...
void TimerContainer::GetIndexTimer(
    std::vector<std::weak_ptr<TimerSlot>>& run_timer_solts,
    std::vector<std::weak_ptr<TimerSlot>>& sub_timer_solts, uint32_t index,
    uint32_t time_pass) {
  ...
  for (auto timer_list = bucket.begin(); timer_list != bucket.end();
       timer_list++) {
    if (timer_list->first == 0) {
      run_timer_solts.insert(run_timer_solts.end(), timer_list->second.begin(),
                             timer_list->second.end());
      continue;
    }
...
void TimerContainer::DoTimer(
    std::vector<std::weak_ptr<TimerSlot>>& run_timer_solts,
    std::vector<std::weak_ptr<TimerSlot>>& sub_timer_solts) {
  // timer call back
  for (auto iter = run_timer_solts.begin(); iter != run_timer_solts.end();
       iter++) {
    auto ptr = iter->lock();
    if (!ptr) {
      continue;
  ...
  for (auto iter = sub_timer_solts.begin(); iter != sub_timer_solts.end();
       iter++) {
    auto ptr = iter->lock();
    if (!ptr) {
      continue;
