...
PoolAlloter::PoolAlloter() : _pool_start(nullptr), _pool_end(nullptr) {
  _free_list.resize(__default_number_of_free_lists);
  memset(&(*_free_list.begin()), 0,
         sizeof(void*) * __default_number_of_free_lists);
...
void* PoolAlloter::MallocAlign(uint32_t size) { return Malloc(Align(size)); }
...
std::shared_ptr<Alloter> MakePoolAlloterPtr() {
  return std::make_shared<burongnet::PoolAlloter>();
}
