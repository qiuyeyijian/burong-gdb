...
BufferBlock::BufferBlock(std::shared_ptr<BlockMemoryPool>& alloter)
    : _can_read(false), _alloter(alloter) {
...
bool BufferBlock::GetFreeMemoryBlock(void*& res1, uint32_t& len1, void*& res2,
                                     uint32_t& len2) {
  res1 = res2 = nullptr;
  len1 = len2 = 0;
...
bool BufferBlock::GetUseMemoryBlock(void*& res1, uint32_t& len1, void*& res2,
                                    uint32_t& len2) {
  res1 = res2 = nullptr;
  len1 = len2 = 0;
...
uint32_t BufferBlock::FindStr(const char* s, uint32_t s_len) {
  if (_write > _read) {
    ...
    const char* find =
        _FindStrInMem(_read, s, uint32_t(_buffer_end - _read), s_len);
    if (find) {
      return uint32_t(find - _read + s_len);
    }
    find = _FindStrInMem(_buffer_start, s, uint32_t(_write - _buffer_start),
                         s_len);
    if (find) {
