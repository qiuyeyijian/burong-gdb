...
BufferQueue::BufferQueue(const std::shared_ptr<BlockMemoryPool>& block_pool,
    ...
    : _can_read_length(0), _block_alloter(block_pool), _alloter(alloter) {}

BufferQueue::~BufferQueue() { _buffer_list.Clear(); }
...
uint32_t BufferQueue::Read(std::shared_ptr<InnerBuffer> buffer, uint32_t len) {
  ...
  while (buffer_read) {
    cur_read_len =
        buffer_read->Read(buffer_queue->_buffer_write, can_write_size);
...
uint32_t BufferQueue::Read(char* res, uint32_t len) {
  ...
  while (buffer_read) {
    total_read_len +=
        buffer_read->Read(res + total_read_len, len - total_read_len);
    if (total_read_len >= len) {
      break;
    }
    if (buffer_read == _buffer_write) {
...
uint32_t BufferQueue::GetCanReadLength() { return _can_read_length; }
