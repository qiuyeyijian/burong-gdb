...
thread_local std::unordered_map<uint64_t, std::shared_ptr<TimerEvent>>
    Dispatcher::__all_timer_event_map;

Dispatcher::Dispatcher(std::shared_ptr<BuRongNetBase> base, uint32_t thread_num,
                       uint32_t base_id)
    : _cur_utc_time(0), _timer_id_creater(0), _cppnet_base(base) {
...
Dispatcher::Dispatcher(std::shared_ptr<BuRongNetBase> base, uint32_t base_id)
    : _cur_utc_time(0), _timer_id_creater(0), _cppnet_base(base) {
  _timer = MakeTimer1Min();
...
void Dispatcher::Listen(uint64_t sock, const std::string& ip, uint16_t port) {
  auto task = [sock, ip, port, this]() {
    ...
    connect_sock->SetBuRongNetBase(_cppnet_base.lock());
...
void Dispatcher::Connect(const std::string& ip, uint16_t port) {
  auto task = [ip, port, this]() {
    ...
    sock->SetBuRongNetBase(_cppnet_base.lock());
...
uint32_t Dispatcher::AddTimer(const user_timer_call_back& cb, void* param,
...
uint32_t Dispatcher::AddTimer(std::shared_ptr<RWSocket> sock, uint32_t interval,
                              bool always) {
  std::shared_ptr<TimerEvent> event = std::make_shared<TimerEvent>();
