...
thread_local std::unordered_map<uint64_t, std::shared_ptr<Socket>>
    RWSocket::__connecting_socket_map;
...
RWSocket::RWSocket(uint64_t sock, std::shared_ptr<AlloterWrap> alloter)
    : Socket(sock),
      ...
      _event(nullptr),
      _alloter(alloter) {
  _block_pool = _alloter->PoolNewSharePtr<BlockMemoryPool>(
      __mem_block_size, __mem_block_add_step);
...
void RWSocket::OnRead(uint32_t len) { Recv(len); }

void RWSocket::OnWrite(uint32_t len) { Send(); }
