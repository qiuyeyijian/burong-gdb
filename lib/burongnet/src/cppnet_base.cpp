...
BuRongNetBase::BuRongNetBase() {}

BuRongNetBase::~BuRongNetBase() {}

void BuRongNetBase::Init(uint32_t thread_num) {
...
void BuRongNetBase::Dealloc() {
  for (size_t i = 0; i < _dispatchers.size(); i++) {
    _dispatchers[i]->Stop();
  }
}

void BuRongNetBase::Join() {
  for (size_t i = 0; i < _dispatchers.size(); i++) {
    _dispatchers[i]->Join();
  }
}

uint64_t BuRongNetBase::AddTimer(uint32_t interval, const user_timer_call_back& cb,
...
void BuRongNetBase::RemoveTimer(uint64_t timer_id) {
  TimerId tid;
  tid._timer_id = timer_id;
  _dispatchers[tid._detail_info._dispatcher_index]->StopTimer(
      tid._detail_info._timer_id);
}

bool BuRongNetBase::ListenAndAccept(const std::string& ip, uint16_t port) {
...
bool BuRongNetBase::Connection(const std::string& ip, uint16_t port) {
  uint32_t index = _random->Random();
  _dispatchers[index]->Connect(ip, port);
  return true;
}

void BuRongNetBase::OnTimer(std::shared_ptr<RWSocket> sock) {
  if (_timer_cb) {
    _timer_cb(sock);
  }
}

void BuRongNetBase::OnAccept(std::shared_ptr<RWSocket> sock) {
  if (_accept_cb) {
    _accept_cb(sock, CEC_SUCCESS);
  }
}

void BuRongNetBase::OnRead(std::shared_ptr<RWSocket> sock,
...
void BuRongNetBase::OnWrite(std::shared_ptr<RWSocket> sock, uint32_t len) {
  if (_write_cb) {
    _write_cb(sock, len);
  }
}

void BuRongNetBase::OnConnect(std::shared_ptr<RWSocket> sock, uint16_t err) {
  if (_connect_cb) {
    _connect_cb(sock, err);
  }
}

void BuRongNetBase::OnDisConnect(std::shared_ptr<RWSocket> sock, uint16_t err) {
  if (_disconnect_cb) {
