...
#include "event/epoll/epoll_action.h"
...
bool EpollEventActions::DelEvent(Event* event) {
  ...
  if (ret < 0) {
    LOG_ERROR("remove event from EPOLL failed! error :%d, socket : %d", errno,
              sock->GetSocket());
    return false;
  }
...
void EpollEventActions::ProcessEvent(int32_t wait_ms) {
  int16_t ret = epoll_wait(_epoll_handler, &*_active_list.begin(),
                           (int)_active_list.size(), wait_ms);
  if (ret == -1) {
    if (errno == EINTR) {
      return;
    }
    ...
    LOG_DEBUG("EPOLL get events! num:%d, TheadId: %ld", ret,
...
void EpollEventActions::OnEvent(std::vector<epoll_event>& event_vec,
                                int16_t num) {
