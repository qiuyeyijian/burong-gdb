...
KqueueEventActions::KqueueEventActions() : _kqueue_handler(-1) {
...
bool KqueueEventActions::AddSendEvent(Event* event) {
  ...
  if (sock) {
    struct kevent ev;
    EV_SET(&ev, sock->GetSocket(), EVFILT_WRITE, EV_ADD | EV_ENABLE | EV_CLEAR,
           0, 0, (void*)event);
...
bool KqueueEventActions::AddRecvEvent(Event* event) {
  ...
  if (sock) {
    struct kevent ev;
    EV_SET(&ev, sock->GetSocket(), EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0,
           (void*)event);
    _change_list.push_back(ev);
    return true;
  }
...
bool KqueueEventActions::AddConnection(Event* event, Address& address) {
  ...
  if (sock) {
    ...
    LOG_ERROR("connect to peer failed! errno:%d, info:%s", ret._errno,
...
bool KqueueEventActions::DelEvent(Event* event) {
  ...
  EV_SET(&read_ev, sock->GetSocket(), EVFILT_READ, EV_DELETE | EV_DISABLE, 0, 0,
         NULL);
  _change_list.push_back(read_ev);

  struct kevent write_ev;
  EV_SET(&write_ev, sock->GetSocket(), EVFILT_WRITE, EV_DELETE | EV_DISABLE, 0,
         0, NULL);
...
void KqueueEventActions::ProcessEvent(int32_t wait_ms) {
  ...
  if (wait_ms > 0) {
    ...
    _kqueue_timeout.tv_nsec =
        ((uint64_t)wait_ms - (_kqueue_timeout.tv_sec * 1000)) * 1000000;

    ret = kevent(_kqueue_handler, &*_change_list.begin(),
                 (int)_change_list.size(), &*_active_list.begin(),
                 (int)_active_list.size(), &_kqueue_timeout);
  } else {
    ret = kevent(_kqueue_handler, &*_change_list.begin(),
                 (int)_change_list.size(), &*_active_list.begin(),
                 (int)_active_list.size(), nullptr);
  }
  ...
  if (ret < 0) {
    ...
    LOG_DEBUG("kevent get events! num:%d, TheadId:%lld", ret,
...
void KqueueEventActions::Wakeup() { write(_pipe[1], "1", 1); }
