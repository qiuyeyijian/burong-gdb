...
#define LOG_DEBUG(log, ...) \
  SingletonLogger::Instance().Debug(__FILE__, __LINE__, log, ##__VA_ARGS__)
#define LOG_INFO(log, ...) \
  SingletonLogger::Instance().Info(__FILE__, __LINE__, log, ##__VA_ARGS__)
#define LOG_WARN(log, ...) \
  SingletonLogger::Instance().Warn(__FILE__, __LINE__, log, ##__VA_ARGS__)
#define LOG_ERROR(log, ...) \
  SingletonLogger::Instance().Error(__FILE__, __LINE__, log, ##__VA_ARGS__)
#define LOG_FATAL(log, ...) \
  SingletonLogger::Instance().Fatal(__FILE__, __LINE__, log, ##__VA_ARGS__)

#define LOG_DEBUG_S                                             \
  LogStream(burongnet::SingletonLogger::Instance().GetStreamParam( \
      burongnet::LL_DEBUG, __FILE__, __LINE__))
#define LOG_INFO_S                                              \
  LogStream(burongnet::SingletonLogger::Instance().GetStreamParam( \
      burongnet::LL_INFO, __FILE__, __LINE__))
#define LOG_WARN_S                                              \
  LogStream(burongnet::SingletonLogger::Instance().GetStreamParam( \
      burongnet::LL_WARN, __FILE__, __LINE__))
#define LOG_ERROR_S                                             \
  LogStream(burongnet::SingletonLogger::Instance().GetStreamParam( \
      burongnet::LL_ERROR, __FILE__, __LINE__))
#define LOG_FATAL_S                                             \
  LogStream(burongnet::SingletonLogger::Instance().GetStreamParam( \
      burongnet::LL_FATAL, __FILE__, __LINE__))
...
class SingletonLogger : public Singleton<SingletonLogger> {
 public:
  ...
  // for log stream
  LogStreamParam GetStreamParam(LogLevel level, const char* file,
                                uint32_t line);
