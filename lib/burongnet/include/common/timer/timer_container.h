...
// if set timeout time is little than accuracy, will be added to sub timer
// wheel. It inherits from the timer interface, so can set another timer wheel
// container to sub timer to support more timer set. More timer define see
// timer.h file.
class TimerContainer : public Timer {
 public:
  TimerContainer(std::shared_ptr<TimerContainer> sub_timer, TIME_UNIT unit,
                 TIME_UNIT max);
  ~TimerContainer();
 ...
 protected:
  ...
  void GetIndexTimer(std::vector<std::weak_ptr<TimerSlot>>& run_timer_solts,
                     std::vector<std::weak_ptr<TimerSlot>>& sub_timer_solts,
                     uint32_t index, uint32_t time_pass);
  void DoTimer(std::vector<std::weak_ptr<TimerSlot>>& run_timer_solts,
               std::vector<std::weak_ptr<TimerSlot>>& sub_timer_solts);
 ...
 protected:
  ...
  std::unordered_map<uint32_t,
                     std::map<uint32_t, std::list<std::weak_ptr<TimerSlot>>>>
      _timer_wheel;
  // std::unordered_map<uint32_t, std::list<std::weak_ptr<TimerSlot>>>
  // _timer_wheel;
};
